@extends('layout.website.master')

@section('title',__('website.volunteer'))

@section('content')

      
        <!-- start of breadcumb -->
        <div class="breadcumb-area">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="breadcumb-wrap">
                            <h2>{{ __('website.trusted_non_profit') }}</h2>
                            <h3>{{ __('website.event_details') }}</h3>
                        </div>
                    </div>
                </div>
            </div>
            <div class="shape">
                <svg width="128" height="357" viewBox="0 0 128 357" fill="none">
                    <path
                        d="M-9.73063 357C-11.832 304.262 -1.65343 268.562 21.3305 247.878C27.4705 242.355 34.4642 238.095 41.2608 233.944C44.6099 231.887 48.0903 229.757 51.4066 227.519C63.6866 219.217 71.3041 210.301 74.5875 200.338C75.1457 198.713 75.5397 196.981 75.7695 195.212C68.2505 198.569 62.1762 199.688 56.5944 198.749C50.5201 197.703 43.5592 193.19 41.6548 185.43C40.3086 179.871 42.2787 173.987 46.974 169.691C52.5229 164.601 60.1405 163.23 65.8865 166.262C74.686 170.882 78.4948 181.639 78.0679 191.783C98.3923 181.603 114.58 162.002 121.77 138.72C129.289 114.318 126.695 86.4868 114.809 64.2508C110.048 55.3348 103.711 46.9963 97.6043 38.9466C86.375 24.1467 74.7845 8.87758 72.6831 -10.3623C70.8116 -28.1583 78.0679 -46.4596 91.2016 -57L92.3508 -55.2312C79.7753 -45.124 72.8473 -27.6529 74.686 -10.6511C76.6889 7.93905 87.6227 22.3419 99.1475 37.5388C105.32 45.6607 111.69 54.0714 116.55 63.1318C128.698 85.8731 131.358 114.39 123.675 139.369C116.221 163.591 99.2131 183.913 77.9366 194.201C77.6739 196.584 77.1814 198.894 76.4591 201.06C72.9786 211.528 65.1313 220.769 52.4573 229.36C49.1082 231.634 45.5949 233.764 42.213 235.821C35.5148 239.9 28.5868 244.16 22.5782 249.538C0.0867844 269.753 -9.82913 304.839 -7.76058 356.856L-9.73063 357ZM59.3525 166.767C55.1825 166.767 51.1111 168.716 48.2873 171.315C44.2159 175.033 42.4757 180.087 43.6249 184.816C45.2994 191.638 51.5379 195.645 56.9556 196.584C62.1762 197.486 67.955 196.367 75.1457 193.154C75.4412 193.01 75.7695 192.866 76.065 192.721C76.7217 182.975 73.1756 172.398 65.0656 168.139C63.2269 167.2 61.2568 166.767 59.3525 166.767Z"
                        fill="white" fill-opacity="0.18" />
                </svg>
            </div>
            <div class="shape-s2">
                <img src="{{ asset('website/images/slider/shape-3.svg') }}" alt="">
            </div>
            <div class="shape-s3">
                <img src="{{ asset('website/images/donate-2.svg') }}" alt="">
            </div>

        </div>
        <!-- end of breadcumb-->

        <!-- event-details-area start -->
        <div class="event-details-area section-padding">
            <div class="container">
                <div class="row">
                    <div class="col col-lg-8">
                        <div class="event-item">
                            <div class="event-img">
                                <img src="{{ asset('website/images/events-single/1.jpg') }}" alt="">
                            </div>
                            <div class="event-details-text">
                                <h2>ساعد الفقراء، ستشعر بالسعادة</h2>
                                <p>من ناحية أخرى، نحن نستنكر وباستنكار شديد ونكره الأشخاص الذين ينخدعون وينحرفون بسبب سحر المتعة اللحظية، ويعمون بسبب الرغبة، فلا يستطيعون توقع الألم والمشاكل التي ستحدث لاحقًا، وينطبق اللوم أيضًا على من يتقاعسون عن أداء واجبهم بسبب ضعف الإرادة، وهو ما يعني الهروب من التعب والألم.</p>
                            </div>
                            <div class="event-details-wrap">
                                <div class="event-details-tab">
                                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                                        <li class="nav-item" role="presentation">
                                            <a class="nav-link active" id="Schedule-tab" data-bs-toggle="tab"
                                                href="#Schedule" role="tab" aria-controls="Schedule"
                                                aria-selected="true">جدول الحدث</a>
                                        </li>
                                        <li class="nav-item" role="presentation">
                                            <a class="nav-link" id="Map-tab" data-bs-toggle="tab" href="#Map" role="tab"
                                                aria-controls="Map" aria-selected="false">موقع الخريطة</a>
                                        </li>
                                        <li class="nav-item" role="presentation">
                                            <a class="nav-link" id="Contact-tab" data-bs-toggle="tab" href="#Contact"
                                                role="tab" aria-controls="Contact" aria-selected="false">اتصل بنا</a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="event-details-content">
                                    <div class="tab-content">
                                        <div id="Schedule" class="tab-pane active">
                                            <p>هذه الحالات بسيطة تمامًا وسهلة التمييز. في ساعة فراغ،
                                                عندما تكون قدرتنا على الاختيار غير مقيدة وعندما لا شيء
                                                يمنعنا من القيام بما نحب أفضل، يجب الترحيب بكل متعة
                                                وتجنب كل ألم.</p>
                                                كل ألم يتم تجنبه.</p>
                                            <ul>
                                                <li>الحكيم في هذه الأمور.</li>
                                                <li>في ساعة فراغ، عندما تكون قدرتنا على الاختيار وعندما لا شيء يمنعنا.</li>
                                                <li>وإلا فإنه يتألم لتجنب الآلام.</li>
                                                <li>نحن نستنكر باستياءٍ صادق كراهية البشر.</li>
                                                <li>وهو ما يعادل القول من خلال ذلك.</li>
                                                <li>لذا فإن الرجل الحكيم يتمسك دائمًا بهذه الأمور.</li>
                                                <li>قوة الاختيار، وعندما لا يوجد شيء.</li>
                                                <li>يتألم لتجنب آلام أسوأ.</li>
                                            </ul>
                                        </div>
                                        <div id="Map" class="tab-pane">
                                            <div class="contact-map">
                                                <iframe
                                                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d193595.9147703055!2d-74.11976314309273!3d40.69740344223377!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x89c24fa5d33f083b%3A0xc80b8f06e177fe62!2sNew+York%2C+NY%2C+USA!5e0!3m2!1sen!2sbd!4v1547528325671"
                                                    allowfullscreen></iframe>
                                            </div>
                                        </div>
                                        <div id="Contact" class="tab-pane">
                                            <div class="event-contact">
                                                <div class="donations-details">
                                                    <form method="post" class="contact-validation-active"
                                                        id="contact-form-main">
                                                        <div class="row">
                                                            <div
                                                                class="col-lg-12 col-md-12 col-sm-12 col-12 form-group">
                                                                <input type="text" class="form-control" name="name"
                                                                    id="name" placeholder="الاسم *">
                                                            </div>
                                                            <div
                                                                class="col-lg-12 col-md-12 col-sm-12 col-12 form-group clearfix">
                                                                <input type="email" class="form-control" name="email"
                                                                    id="email" placeholder="البريد الإلكتروني الخاص بك">
                                                            </div>
                                                            <div class="col-lg-12 col-12 form-group">
                                                                <textarea class="form-control" name="note" id="note"
                                                                    placeholder="رسالة"></textarea>
                                                            </div>
                                                            <div class="submit-area col-lg-12 col-12">
                                                                <button type="submit"
                                                                    class="theme-btn submit-btn">إرسال الآن</button>
                                                                <div id="loader">
                                                                    <i class="ti-reload"></i>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="clearfix error-handling-messages">
                                                           <div id="success">شكرًا لك</div>
                                                            <div id="error">حدث خطأ أثناء إرسال البريد الإلكتروني. يرجى المحاولة مرة أخرى لاحقًا.</div>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col col-lg-4 col-12">
                        <div class="blog-sidebar">
                            {{-- <div class="widget search-widget">
                                <h3>search here</h3>
                                <form>
                                    <div>
                                        <input type="text" class="form-control" placeholder="Search Post..">
                                        <button type="submit"><i class="ti-search"></i></button>
                                    </div>
                                </form>
                            </div> --}}
                            <div class="widget recent-post-widget">
                                <h3>منشورات ذات صلة</h3>
                                <div class="posts">
                                    <div class="post">
                                        <div class="img-holder">
                                            <img src="{{ asset('website/images/recent-posts/img-1.jpg') }}" alt>
                                        </div>
                                        <div class="details">
                                            <span class="date"><i class="flaticon-calendar"></i>30 Nov 2024 </span>
                                            <h4><a href="{{ route('event-details') }}">مشاكل الميزانية تجبرنا على التغيير</a></h4>
                                        </div>
                                    </div>
                                    <div class="post">
                                        <div class="img-holder">
                                            <img src="{{ asset('website/images/recent-posts/img-2.jpg') }}" alt>
                                        </div>
                                        <div class="details">
                                            <span class="date"><i class="flaticon-calendar"></i>30 Nov 2024 </span>
                                            <h4><a href="{{ route('event-details') }}">أفضل المنتجات التي تشكل الموضة</a></h4>
                                        </div>
                                    </div>
                                    <div class="post">
                                        <div class="img-holder">
                                            <img src="{{ asset('website/images/recent-posts/img-3.jpg') }}" alt>
                                        </div>
                                        <div class="details">
                                            <span class="date"><i class="flaticon-calendar"></i>30 Nov 2024 </span>
                                            <h4><a href="{{ route('event-details') }}">أفضل المنتجات التي تشكل الموضة</a></h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="widget tag-widget">
                                <h3>الوسوم الشائعة</h3>
                                <ul>
                                    <li><a href="#">العطاء</a></li>
                                    <li><a href="#">جهود الإغاثة</a></li>
                                    <li><a href="#">أثر إيجابي</a></li>
                                    <li><a href="#">لطف</a></li>
                                    <li><a href="#">مساعدة</a></li>
                                    <li><a href="#">رعاية</a></li>
                                </ul>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- event-details-area end -->

@endsection