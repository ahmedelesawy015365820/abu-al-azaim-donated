        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6 mb-2" v-if="submitData.data.ar" v-for="lang in languages">
                        ...
                        <input type="text" class="form-control form-control-lg"  v-model="v$.link.$model"
                               :class="{'is-invalid': v$.link.$error || errors[`link`],
                                   'is-valid': !v$.link.$invalid && !errors[`link`]}">

                        <div class="invalid-feedback">
                          <span v-if="v$.link.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                        </div>
                        <template v-if="errors[`link`]">
                            <error-message v-for="(errorMessage, index) in errors[`link`]" :key="index">
                              {{ errorMessage }}
                            </error-message>
                          </template>
                      </div>
                        ...
                        <div class="col-md-12 mt-3">
                          <label class="form-label">صورة  (957 * 678)</label>
  ...
  function defaultData(){
    ...
    languages.value.forEach((el)=>{
      ...
      langValidation.value[el] ={
        ...
        description: {minLength: minLength(1),maxLength:maxLength(1000),required,}
      }
    });
  ...
  function resetModal() {
    ...
    setTimeout(async () => {
      if (props.type != 'edit') {
        ...
        adminApi.get(`two-abouts/${id.value}`)
            .then((res) => {
              loading.value = true;
              let l = res.data.data;
    ...
    formData.append('link', submitData.data.link);
      if (props.type !== 'edit') {
        if (!v$.value.$error) {
          is_disabled.value = false;
          loading.value = true;
          adminApi.post(`two-abouts`, formData)
              .then((res) => {
                Swal.fire({
                  icon: 'success',
        ...
        adminApi.post(`two-abouts/${id.value}`,formData)
            .then((res) => {
              Swal.fire({
                icon: 'success',
